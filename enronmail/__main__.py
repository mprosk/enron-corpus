...
def main():
    ...
    init_db_parser = subparsers.add_parser("init-db", help="Initialize the SQL database")
    init_db_parser.add_argument("--maildir", type=str, default="maildir", 
                               help="Path to the maildir directory containing the emails")
    init_db_parser.add_argument("--pickle-path", type=str, default="pickle/file_list.pickle",
                               help="Path to the pickle file containing the file list")
    init_db_parser.add_argument("--generate-file-list", action="store_true",
    ...
    if args.command == "init-db":
        ...
        if args.generate_file_list:
            file_list = analysis.generate_file_list(Path(args.maildir), args.pickle_path)
        else:
